import { buildAssistantInput } from "./wallet-copilot";
...
function getProvider() {
  ...
  const runtimeOidc = process.env.VERCEL_OIDC_TOKEN || "";
  if (runtimeOidc) {
    return {
      apiKey: runtimeOidc,
      apiUrl: GATEWAY_API_URL,
...
function toolCallToAction(item) {
  ...
  if (item.name === "prepare_bridge") {
    return { id, kind: "wallet-action", tool: item.name, label: `Review ${args.amount || ""} USDC bridge`.trim(), args };
  }
  ...
  try {
    const response = await fetch(provider.apiUrl, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          "You are Arc AI Agent inside a self-custodial wallet.",
          ...
          "For wallet-specific claims, use the supplied wallet context and never invent balances, activity, transaction status, or risk facts.",
...
export async function handleWalletChat(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({
      ...
      provider: provider?.provider || "client-free-ai",
  ...
  if (!provider) {
    return res.status(503).json({
      error: "Server AI provider is not configured. Use the in-browser free AI fallback.",
  ...
  try {
    ...
    return res.status(200).json({
      ...
      notice: `Arc Agent · ${provider.model}`,
      mode: "ai-copilot",
      provider: provider.provider
    });
  } catch (error) {
    console.error("[wallet-copilot] provider unavailable", {
      provider: provider.provider,
    ...
    return res.status(503).json({
      error: "Server AI is unavailable right now. Switching to the real in-browser free AI provider is required.",
      code: "AI_PROVIDER_UNAVAILABLE",
      fallback: "puter-client",
      provider: provider.provider
    });
