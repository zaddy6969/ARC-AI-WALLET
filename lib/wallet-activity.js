...
function getArcRpcUrl() {
  ...
  if (!rpcUrl) {
    throw new Error(`${arcTestnet?.name || "Arc"} RPC is not configured.`);
  }
  return rpcUrl;
}
...
function mapExplorerNativeTransaction(transaction, walletAddress) {
  ...
  const fromAddress = getExplorerAddress(transaction?.from);
  const from = fromAddress ? normalizeAddress(fromAddress) : "";
...
async function getExplorerNativeActivity(walletAddress) {
  ...
  const url = `${explorerUrl}/api/v2/addresses/${walletAddress}/transactions`;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Arc explorer returned ${response.status}`);
    }

    const payload = await response.json();
    const items = Array.isArray(payload?.items) ? payload.items : [];

    debugActivityLog("arc-explorer-native-transactions", {
      walletAddress,
    ...
    debugActivityLog("arc-explorer-native-error", {
      walletAddress,
      network: ARC_NETWORK_LABEL,
