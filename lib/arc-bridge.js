...
export function formatBridgeError(error) {
  ...
  if (normalized.includes("forward") && (normalized.includes("unsupported") || normalized.includes("not available"))) {
    return "Circle Forwarding Service is not available for this exact route right now. Try the reverse route or another supported network.";
  }
...
function getBridgeChainLookup(appKit, chainModule) {
  ...
  try {
    ...
    // Exported App Kit definitions remain a safe fallback.
  }
  return lookup;
}
...
function buildBridgeDestination({ adapter, chainLookup, to }) {
  ...
  const useForwarder = Boolean(
    to.useForwarder || destinationDefinition?.cctp?.forwarderSupported?.destination
  );
