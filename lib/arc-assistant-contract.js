...
import { ARC_MAINNET_REQUESTED, ARC_TESTNET_CHAIN_ID, arcActiveChain } from "./arc-chain";
...
const verifiedGeneratedTestnetAddress =
  deployment?.verification?.status === "verified" &&
  deployment?.verification?.receiptStatus === 1 &&
  Number(deployment?.chainId) === ARC_TESTNET_CHAIN_ID &&
  /^0x[a-fA-F0-9]{40}$/.test(deployment?.address || "")
    ? deployment.address
    : "";
...
export const LUMEXA_WALLET_ASSISTANT_CONTRACT_ADDRESS = ARC_MAINNET_REQUESTED
  ...
  : process.env.NEXT_PUBLIC_LUMEXA_ASSISTANT_CONTRACT_ADDRESS || verifiedGeneratedTestnetAddress;
