...
const LEGACY_STORAGE_KEYS = ["lumexa-ai-wallet:activity:v1", "lumexa-ai-wallet:activity", "arc-ai-wallet:activity"];
...
function readStorage() {
  ...
  try {
    ...
    const legacy = LEGACY_STORAGE_KEYS.flatMap((key) => parseStoredList(window.localStorage.getItem(key)));
    ...
    [...current, ...legacy].forEach((item) => {
      ...
      const key = txHash ? `tx:${txHash}` : id ? `id:${id}` : `legacy:${item?.type || "activity"}:${item?.createdAt || ""}:${item?.amount || ""}`;
...
function semanticKind(item) {
  ...
  if (kind === "received" || type.startsWith("received") || type.includes("receive")) return "received";
  return kind;
}
...
function mergeActivityItemPair(left, right) {
  ...
  const kind = preserveAppMeaning ? appSemanticKind : semanticKind(chainItem) || semanticKind(appItem) || semanticKind(primary);

  return {
    ...primary,
    id: chainItem?.id || appItem?.id || primary.id,
    source: chainItem && appItem ? "merged" : primary.source,
    walletAddress: appItem?.walletAddress || chainItem?.walletAddress || primary.walletAddress || "",
    type: preserveAppMeaning ? appItem?.type || primary.type : chainItem?.type || appItem?.type || primary.type,
    kind,
    amount: preserveAppMeaning ? appItem?.amount || chainItem?.amount || primary.amount || "" : chainItem?.amount || appItem?.amount || primary.amount || "",
    ...
    sender: chainItem?.sender || appItem?.sender || chainItem?.from || primary.sender || primary.from || "",
    receiver: chainItem?.receiver || appItem?.receiver || chainItem?.to || primary.receiver || primary.to || "",
    recipient: chainItem?.recipient || appItem?.recipient || chainItem?.counterparty || primary.recipient || "",
    ...
    txHashShort: chainItem?.txHashShort || appItem?.txHashShort || shortHash(chainItem?.txHash || appItem?.txHash || primary.txHash || ""),
    explorerUrl: chainItem?.explorerUrl || appItem?.explorerUrl || primary.explorerUrl || "",
    summary: preserveAppMeaning ? appItem?.summary || chainItem?.summary || primary.summary || "" : chainItem?.summary || appItem?.summary || primary.summary || "",
    createdAt: appItem?.createdAt || chainItem?.createdAt || primary.createdAt || new Date().toISOString(),
    ...
    metadata: { ...(chainItem?.metadata || {}), ...(primary.metadata || {}), ...(appItem?.metadata || {}) }
  };
}

export function saveLocalActivity(item) {
  const current = readStorage();
  const normalizedItem = { ...item, walletAddress: normalizeAddress(item?.walletAddress), kind: semanticKind(item) || item?.kind || "" };
  ...
  debugActivityLog("local-save", { txHash: item?.txHash || "", type: item?.type || "", kind: nextItem.kind });
...
export function updateLocalActivityStatuses(statuses) {
  ...
  const nextItems = current.map((item) => {
    const key = Object.keys(statuses).find((hash) => String(hash).toLowerCase() === String(item.txHash || "").toLowerCase());
    const nextStatus = key ? statuses[key]?.status : "";
    if (!nextStatus || nextStatus === item.status) return item;
    changed = true;
    return { ...item, status: nextStatus, blockNumber: statuses[key]?.blockNumber || item.blockNumber || null };
  });
...
export function createWalletActionRecord({ walletAddress, type, kind = "", amount, chain, chainId = null, token = "USDC", sender = "", receiver = "", recipient = "", status = "Confirmed", txHash = "", explorerUrl = "", summary = "", metadata = {} }) {
...
export function mergeActivityFeedItems(localItems, liveItems) {
  ...
  return [...merged.values()].sort((left, right) => getTimestamp(right.createdAt) - getTimestamp(left.createdAt));
}
...
export function useLocalActivityHistory(address) {
  ...
  useEffect(() => {
    ...
    const handleStorage = (event) => {
      if (!event.key || event.key === STORAGE_KEY || LEGACY_STORAGE_KEYS.includes(event.key)) refresh();
    };
  ...
  return useMemo(() => ({ items, save: saveLocalActivity, updateStatuses: updateLocalActivityStatuses, updateByHash: updateLocalActivityByHash, refresh }), [items, refresh]);
}
