...
function getActivityKey(item) {
  ...
  return `fallback:${[item?.type, item?.createdAt, item?.amount, item?.walletAddress]
    .filter(Boolean)
    .join(":")}`;
}
...
export function saveLocalActivity(item) {
  ...
  const withoutDuplicate = current.filter((existing) => getActivityKey(existing) !== key);
  debugActivityLog("local-save", { txHash: item?.txHash || "", type: item?.type || "", kind: normalizedItem.kind });
  writeStorage([normalizedItem, ...withoutDuplicate].slice(0, 150));
}
...
export function useLocalActivityHistory(address) {
  ...
  const refresh = useCallback(() => {
    ...
    const nextItems = storedItems.filter((item) => walletAddress && normalizeAddress(item.walletAddress) === walletAddress);
    setItems(nextItems);
  }, [address]);
